...
`PATH`, because Hydra invokes `git` directly. The ZIP also contains the
ready-generated `completions/hydra.bash` registration described below.
...
The native Windows ZIP is portable and therefore does not edit `.bashrc` or
register completion automatically. After extracting it, add this to Git Bash's
`~/.bashrc`, assuming the archive directory itself is on `PATH`:

```bash
source "$(dirname "$(command -v hydra.exe)")/completions/hydra.bash"
```

For a source installation, a release archive without a packaged script, or
when Tab completion is still unavailable, load the registration generated by
the currently installed executable explicitly at shell startup.
