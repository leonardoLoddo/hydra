...
| [`head-close.md`](head-close.md) | `hydra head close`, parent-worktree invocation, native Git merge output, conflict resolution or abort, target validation, configured adapters, or removal after integration | Implemented close transaction, parent-worktree Git integration, conflict continuation and abort behavior, adapter execution, and protected-removal composition. |
