...
The public skill-management hierarchy uses an explicit provider:
...
Hydra's first-party `hydra skill install <provider>` hierarchy owns the desired
interactive installation experience. When run on an interactive terminal it
must:

1. explain that skill installation for the selected provider is optional;
2. show the resolved provider destination and ask for confirmation with a
...
Codex currently discovers personal skills below `$HOME/.agents/skills`, so its
adapter installs Hydra at `$HOME/.agents/skills/hydra`. This host contract must
be revalidated against the
...
- respect the selected provider's documented personal-skill location;
...
- state how the selected provider refreshes skill discovery;
- keep every canonical file under `skills/hydra/` identical to the verified
  release artifact rather than maintaining an installer-specific fork.

Each provider adapter may add a `.hydra-skill.json` provenance manifest beside
the portable artifact. It records the provider, Hydra version, and SHA-256
digest of every canonical installed file. `update` and `remove` must validate
that manifest and the exact installed tree before mutation; an unknown origin,
...
When the canonical skill adds bundled files, the newer adapter MAY recognize a
previous exact managed layout so `hydra skill update` can replace it atomically.
Compatibility MUST require the previous manifest's provider, version, file set,
and checksums to match the installed tree. It MUST NOT accept missing files from
the current layout as an unmodified current installation or weaken protection
for unknown and locally changed content.
